...
<p style="text-align: right;"><strong><em>Ảnh: Trịnh Phương Thu &ndash; VPCT</em></strong></p>
  ...
  // Tìm bài viết theo slug - sử dụng useMemo để tránh tính toán lại
  const newsItem = useMemo(() => newsData.find((item) => item.slug === slug), [slug]);

  // Lọc bài viết cho sidebar theo tìm kiếm - sử dụng useMemo
  ...
  // Lọc bài viết liên quan - sử dụng useMemo
  ...
  useEffect(() => {
    ...
    setIsInitialized(true);
  }, [newsItem]); // Chỉ chạy lại khi newsItem thay đổi
