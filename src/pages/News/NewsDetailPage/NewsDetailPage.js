...
const NewsDetailPage = () => {
  ...
  useEffect(() => {
    const images = document.querySelectorAll('.news-detail-page .article-content img[loading="lazy"]');
  ...
  const filteredNews = useMemo(() => {
    if (!newsItem) return [];
    const categorySlugField = isEnglish ? 'postCategorySlugEn' : 'postCategorySlugVi';
    const targetCategorySlug = isEnglish ? newsItem.postCategorySlugEn : newsItem.postCategorySlugVi;
    
    let filtered = mockNews.filter(
      n => n[categorySlugField] === targetCategorySlug &&
        ((isEnglish ? n.titleEn : n.titleVi).toLowerCase().includes(searchTerm.toLowerCase()) ||
         (isEnglish ? n.contentEn : n.contentVi).toLowerCase().includes(searchTerm.toLowerCase()))
    );
  ...
  const relatedNews = useMemo(() => {
    ...
    return mockNews
      .filter(n => n.id !== newsItem.id && n[categorySlugField] === targetCategorySlug)
      .sort((a, b) => new Date(b.timePosted) - new Date(a.timePosted))
      .slice(0, 3);
  }, [newsItem, isEnglish]);

  const formatDate = (dateString) => {
    return new Date(dateString).toLocaleDateString(currentLanguage === 'vi' ? 'vi-VN' : 'en-GB', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
  ...
  const getDescription = () => isEnglish ? newsItem?.descriptionEn : newsItem?.descriptionVi;
  const getContent = () => isEnglish ? newsItem?.contentEn : newsItem?.contentVi;
  const getCategoryName = () => isEnglish ? newsItem?.postCategoryNameEn : newsItem?.postCategoryNameVi;
  const getCategorySlug = () => isEnglish ? newsItem?.postCategorySlugEn : newsItem?.postCategorySlugVi;
  ...
  return (
    <>
      <SEO 
        ...
        url={currentLanguage === 'vi' ? `/tin-tuc/${categorySlug}/${newsSlug}` : `/en/news/${categorySlug}/${newsSlug}`}
      />
      <div className="news-detail-page">
      <div className="news-detail-layout">
          ...
          <ul>
            {filteredNews.length > 0 ? (
              filteredNews.map(n => (
                <li key={n.id}>
                  <Link
                    to={currentLanguage === 'vi' ? `/tin-tuc/${n.postCategorySlugVi}/${n.slugVi}` : `/en/news/${n.postCategorySlugEn}/${n.slugEn}`}
        ...
        <div className="news-detail-container">
          ...
          <nav className="breadcrumb-nav">
            <LocalizedLink routeKey="HOME">{tNews('navigation.home')}</LocalizedLink> &gt;{' '}
            <LocalizedLink routeKey="NEWS">{tNews('navigation.news')}</LocalizedLink> &gt;{' '}
            <LocalizedLink to={currentLanguage === 'vi' ? `/tin-tuc/${getCategorySlug()}` : `/en/news/${getCategorySlug()}`}>
              {getCategoryName()}
            </LocalizedLink> &gt;{' '}
            <span>{getTitle()}</span>
          </nav>
          ...
          <article className="news-article">
            ...
            <footer className="article-footer">
              <div className="article-tags">
                <span className="tag-label">{tNews('frontend.news.category')}:</span>
                <LocalizedLink to={currentLanguage === 'vi' ? `/tin-tuc/${getCategorySlug()}` : `/en/news/${getCategorySlug()}`} className="tag">
                  {getCategoryName()}
                </LocalizedLink>
              </div>
                  ...
                  <div className="related-news-item" key={n.id}>
                    ...
                    >
                      <div className="related-news-thumb">
                        <img src={n.image} alt={currentLanguage === 'vi' ? n.titleVi : n.titleEn} />
                      </div>
                      <div className="related-news-info">
                        <h4>{currentLanguage === 'vi' ? n.titleVi : n.titleEn}</h4>
